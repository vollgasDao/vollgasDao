...
contract('RinkebyGasStation', accounts => {
  const LinkToken = artifacts.require('LinkToken.sol')
  const Oracle = artifacts.require('Oracle.sol')
  const RinkebyGasStation = artifacts.require('RinkebyGasStation.sol')
  ...
  beforeEach(async () => {
    ...
    rgs = await RinkebyGasStation.new({ from: consumer })
  ...
  describe('#fulfill', () => {
    ...
    it('records the data given to it by the oracle', async () => {
      const currentPrice = await rgs.data.call()
      assert.equal(
    ...
    context('when called by anyone other than the oracle contract', () => {
      it('does not accept the data provided', async () => {
        await h.assertActionThrows(async () => {
          await rgs.fulfill(request.id, response, { from: stranger })
        })
      })
    })
  })
  ...
  describe('#withdrawLink', () => {
    ...
    context('when called by a non-owner', () => {
      it('cannot withdraw', async () => {
        await h.assertActionThrows(async () => {
          await rgs.withdrawLink({ from: stranger })
        })
      })
    })
    ...
    context('when called by the owner', () => {
      it('transfers LINK to the owner', async () => {
        ...
        await rgs.withdrawLink({ from: consumer })
